...
4. In **Authentication -> Email Templates -> Confirm signup**, paste the HTML from
   `supabase/templates/confirmation.html` and set the subject to
   `Confirm your AxleLift account` (the same settings live in `supabase/config.toml`
   for local development).
5. Restart Expo with cache clear (`npx expo start -c`).
6. Sign up in the app with a real account — the demo banner disappears once credentials are valid.
